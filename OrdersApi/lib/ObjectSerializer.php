...
namespace PrioTicket\DistributorAPI;

use PrioTicket\DistributorAPI\Models\ModelInterface;
use Session;
...
class ObjectSerializer
{
     ...
     */
    public static function deserialize($data, $class, $httpHeaders = null , $orderId='')
    {
        if (null === $data) {
            return null;
        }
        ...
        if (strcasecmp(substr($class, -2), '[]') === 0) {
            ...
            foreach ($data as $key => $value) {
                $values[] = self::deserialize($value, $subClass, null ,$orderId);
            }
            return $values;
        }
        ...
        if (substr($class, 0, 4) === 'map[') { // for associative array e.g. map[string,int]
            ...
            if (strrpos($inner, ",") !== false) {
                ...
                foreach ($data as $key => $value) {
                    $deserialized[$key] = self::deserialize($value, $subClass, null ,$orderId);
                }
            }
            return $deserialized;
        }
        ...
        if ($class === '\SplFileObject') {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\PrioTicket\DistributorAPI\Model\\' . $data->{$discriminator};
            ...
            foreach ($instance::openAPITypes() as $property => $type) {
                ...
                if (isset($data->{$instance::attributeMap()[$property]})) {
                    ...
                    if(is_string($propertyValue) && $property=='order_reference')
                    {
                        $orderId = $propertyValue;
                    }
                    
                    try
                    {
                        $instance->$propertySetter(self::deserialize($propertyValue, $type, null , $orderId));
                    }catch (\InvalidArgumentException $e) {
                        if (Session::has('sdkError')) 
                        {
                            $sdkError = session()->get('sdkError');
                        }else
                        {
                            $sdkError = [];
                        }
                        $sdkError[$orderId][] = $e->getMessage();
                        $sdkError[$orderId] = array_unique($sdkError[$orderId]);
                        session(['sdkError' => $sdkError]);
                    }
                }
            }
            return $instance;
